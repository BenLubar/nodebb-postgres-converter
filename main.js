...
async function main(reader, input, output, concurrency, memory) {
	...
	await Promise.all([
		...
		query('Insert into legacy_zset', pool, 'INSERT INTO "legacy_zset" ("_key", "value", "score") SELECT "data"->>\'_key\', "data"->>\'value\', ("data"->>\'score\')::numeric FROM "objects" WHERE (SELECT COUNT(*) FROM jsonb_object_keys("data" - \'expireAt\')) = 3 AND ("data" ? \'value\') AND ("data" ? \'score\') AND ("data"->>\'value\' IS NOT NULL) AND ("data"->>\'score\' IS NOT NULL)'),
