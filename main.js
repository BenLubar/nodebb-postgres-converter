...
async function main(reader, input, output, concurrency, memory) {
	...
	await transaction('Objects', pool, async function(db) {
		...
		await query('Insert into legacy_object (zset)', db, 'INSERT INTO "legacy_object" ("_key", "type", "expireAt") SELECT "data"->>\'_key\', \'zset\'::LEGACY_OBJECT_TYPE, MIN(CASE WHEN ("data" ? \'expireAt\') THEN to_timestamp(("data"->>\'expireAt\')::double precision / 1000) ELSE NULL END) FROM "objects" WHERE ("data" ? \'score\') AND ("data"->>\'value\' IS NOT NULL) AND ("data"->>\'score\' IS NOT NULL) GROUP BY "data"->>\'_key\'');
	...
	await Promise.all([
		query('Insert into legacy_hash', pool, 'INSERT INTO "legacy_hash" ("_key", "data") SELECT l."_key", o."data" - \'_key\' - \'expireAt\' FROM "legacy_object" l INNER JOIN "objects" o ON l."_key" = o."data"->>\'_key\' WHERE l."type" = \'hash\''),
		query('Insert into legacy_zset', pool, 'INSERT INTO "legacy_zset" ("_key", "value", "score") SELECT l."_key", o."data"->>\'value\', (o."data"->>\'score\')::numeric FROM "legacy_object" l INNER JOIN "objects" o ON l."_key" = o."data"->>\'_key\' WHERE l."type" = \'zset\' AND o."data"->>\'value\' IS NOT NULL AND o."data"->>\'score\' IS NOT NULL'),
		query('Insert into legacy_set', pool, 'INSERT INTO "legacy_set" ("_key", "member") SELECT l."_key", jsonb_array_elements_text(o."data"->\'members\') FROM "legacy_object" l INNER JOIN "objects" o ON l."_key" = o."data"->>\'_key\' WHERE l."type" = \'set\''),
		query('Insert into legacy_list', pool, 'INSERT INTO "legacy_list" ("_key", "array") SELECT l."_key", ARRAY(SELECT a.t FROM jsonb_array_elements_text(o."data"->\'list\') WITH ORDINALITY a(t, i) ORDER BY a.i ASC) FROM "legacy_object" l INNER JOIN "objects" o ON l."_key" = o."data"->>\'_key\' WHERE l."type" = \'list\''),
		query('Insert into legacy_string', pool, 'INSERT INTO "legacy_string" ("_key", "data") SELECT l."_key", CASE WHEN o."data" ? \'value\' THEN o."data"->>\'value\' ELSE o."data"->>\'data\' END FROM "legacy_object" l INNER JOIN "objects" o ON l."_key" = o."data"->>\'_key\' WHERE l."type" = \'string\'')
	]);
	...
	try {
		await query('Alter tables cluster on', db, 'ALTER TABLE "legacy_object" CLUSTER ON "legacy_object_pkey"; ALTER TABLE "legacy_hash" CLUSTER ON "legacy_hash_pkey"; ALTER TABLE "legacy_zset" CLUSTER ON "legacy_zset_pkey"; ALTER TABLE "legacy_set" CLUSTER ON "legacy_set_pkey"; ALTER TABLE "legacy_list" CLUSTER ON "legacy_list_pkey"; ALTER TABLE "legacy_string" CLUSTER ON "legacy_string_pkey"');
